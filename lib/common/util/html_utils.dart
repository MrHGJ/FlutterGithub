...
class HtmlUtils {
  static generateCode2HTml(String mdData,
      {String backgroundColor = MyColors.miWhiteString,
      String lang = 'java',
      userBR = true}) {
  ...
  static generateHtml(String mdData,
      {String backgroundColor = MyColors.webDraculaBackgroundColorString,
      userBR = true}) {
    if (mdData == null) {
      return "";
    }
    ...
    return generateCodeHtml(mdDataCode, false,
        ...
        actionColor: MyColors.actionBlueString,
        userBR: userBR);
  }
  ...
  static generateCodeHtml(mdHTML, wrap,
      ...
      String actionColor = MyColors.actionBlueString,
      userBR = true}) {
    return "<html>\n" +
        "<head>\n" +
        "<meta charset=\"utf-8\" />\n" +
  ...
  static resolveHtmlFile(var res, String defaultLang) {
    if (res != null ) {
      ...
      if (startLang >= 0 && endLang >= 0) {
        String tmpLang =
            res.substring(startLang + startTag.length, endLang);
        if (tmpLang != null) {
      ...
      if ('markdown' == lang) {
        return generateHtml(res, backgroundColor: MyColors.miWhiteString);
      } else {
        return generateCode2HTml(res,
